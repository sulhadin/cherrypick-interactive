    ...
    try {
        ...
        if (argv['semantic-versioning']) {
            if (!argv['current-version']) {
                throw new Error(' --semantic-versioning requires --current-version X.Y.Z')
            }
            ...
            // If you want to stop on "none", uncomment:
            // if (!bump) { console.log(chalk.yellow("No semver-relevant commits.")); return; }
        }
        ...
        if (argv['create-release']) {
            ...
            const versionForBranch = computedNextVersion // must exist now
            if (!versionForBranch) {
                throw new Error('Unable to determine release version. Check semantic-versioning inputs.')
            }

            const releaseBranch = `release/${versionForBranch}`
            ...
            await createReleaseBranchFrom(startPoint, releaseBranch, {
                push: argv['push-release']
            })
...
async function createReleaseBranchFrom(startPointRef, releaseBranch, { push }) {
    // Make the branch from the given start point (can be 'origin/main')
    await git.checkoutBranch(releaseBranch, startPointRef)
    if (push) {
        await git.push(['-u', 'origin', releaseBranch])
    }
}

async function buildChangelogBody({ version, baseBranch, headBranch, hashes, gitRawFn }) {
    const today = new Date().toISOString().slice(0, 10)
    const header = version ? `## Release ${version} — ${today}` : `## Release — ${today}`
    const meta = `**Base:** \`${baseBranch}\`  \n**Head:** \`${headBranch}\``
    ...
    for (const h of hashes) {
        // Use the commit's actual message (conventional commit), not the PR title
        const full = await gitRawFn(['show', '--format=%B', '-s', h])
        const subject = full.split(/\r?\n/)[0].trim() // first line of commit message
        const s = shortSha(h)
        const shaDisplay = `(${s})`
        ...
        if (/^feat[:(]/i.test(subject)) {
            ...
            chores.push(`- ${shaDisplay} ${subject}`)
        } else {
